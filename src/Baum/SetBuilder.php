...
class SetBuilder
{
     ...
     * @return void
     */
    public function rebuild($force = false, $withTrashed = false)
    {
        ...
        $this->node->getConnection()->transaction(function () use ($self, $withTrashed) {
            foreach ($self->roots() as $root)
                $self->rebuildBounds($root, 0, $withTrashed);
        });
    }
     ...
     */
    public function rebuildBounds($node, $depth = 0, $withTrashed = false)
    {
        ...
        foreach ($this->children($node, $withTrashed) as $child)
            $this->rebuildBounds($child, $depth + 1, $withTrashed);
     ...
     * @return  Illuminate\Database\Eloquent\Collection
     */
    public function children($node, $withTrashed = false)
    {
